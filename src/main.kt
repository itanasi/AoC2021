...
fun distanceToFuel(distance: Int): Int
{
    if (distance == 0)
        return 0
    return distanceToFuel(distance-1)+distance
}
...
fun main() {
    ...
    var minFuel = crabPos.sumOf { distanceToFuel(it) }
    for (pos in 1..crabPos.maxOf{ it }){
        val fuel = crabPos.map{ distanceToFuel(abs(it - pos)) }.sumOf { it }
        if (fuel < minFuel){
            minPos = pos
            minFuel = fuel
        }
