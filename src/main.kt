...
fun main() {
    ...
    val lineSegments = lines.map{it.split(" -> ").map{it.split(",")}}

    var grid : HashMap<String, Int> = HashMap<String, Int>()

    for (segment in lineSegments){
        var x1 = segment[0][0].toInt()
        var y1 = segment[0][1].toInt()
    ...
    val intersections = grid.filter{it.value > 1}.size
